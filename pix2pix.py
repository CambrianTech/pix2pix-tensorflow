...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        if len(combined_names) > 0:
            ...
            a_images = model.preprocess(raw_input[:,:width//2,:])
            b_images = model.preprocess(raw_input[:,width//2:,:])
...
# you will want to use tf.concat to combine them together.
            ...
            a_images = model.preprocess(raw_input_a)
            b_images = model.preprocess(raw_input_b)
...
def create_model(inputs, targets):
    ...
    with tf.variable_scope("generator"):
        ...
        outputs = model.create_generator(a, inputs, out_channels)
