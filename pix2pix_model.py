...
class Pix2PixModel(cambrian.nn.ModelBase):
    ...
    def _setup_train(self):
        ...
        with tf.name_scope("real_discriminator"):
            with tf.variable_scope("discriminator", use_resource=True):
                # 2x [batch, height, width, channels] => [batch, 30, 30, 1]
                predict_real = create_discriminator(self.args, self.inputs, self.targets)

        with tf.name_scope("fake_discriminator"):
            with tf.variable_scope("discriminator", reuse=True, use_resource=True):
                # 2x [batch, height, width, channels] => [batch, 30, 30, 1]
        ...
        for spec in self.args["a_specs"]:
            with tf.name_scope("inputs_summary"):
                tf.summary.image("inputs_%d" % spec.index, self.inputs[:, :, :, spec.start_channel:spec.start_channel+spec.channels])
            with tf.name_scope("outputs_summary"):
                tf.summary.image("outputs_%d" % spec.index, self.outputs[:, :, :, spec.start_channel:spec.start_channel+spec.channels])

        for spec in self.args["b_specs"]:
            with tf.name_scope("targets_summary"):
                tf.summary.image("targets_%d" % spec.index, self.targets[:, :, :, spec.start_channel:spec.start_channel+spec.channels])
